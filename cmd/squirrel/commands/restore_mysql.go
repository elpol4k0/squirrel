...
var mysqlRestoreCmd = &cobra.Command{
  ...
  squirrel restore mysql abc123 --repo /backup/myrepo --dsn "root:pw@tcp(host:3306)/" --binlog-dir /tmp/binlogs`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		return runMySQLRestore(repoPath, args[0], dsn, binlogDir, sqlOnly)
	},
}
...
func runMySQLRestore(repoURL, snapID, dsn, binlogDir string, sqlOnly bool) error {
	...
	if snap.Meta["type"] != "mysql-dump" {
		return fmt.Errorf("snapshot %s is not a mysql-dump snapshot (type=%q)", snap.ID[:8], snap.Meta["type"])
	}
	...
	// Collect binlog snapshots after the dump
